...
<?php
    ...
    /* = -------------------------------------------------------------------------- = */
     ...
     include_once(G5_MCONTENTS_PATH.'/settle_kcp.inc.php');       // 환경설정 파일 include
?>
<?php
    /* = -------------------------------------------------------------------------- = */
...
<?php
    ...
    if(get_session('ss_cm_cash_charge_id')) {
        $js_return_url = G5_CONTENTS_URL.'/cashform.php';
    } else {
        $js_return_url = G5_CONTENTS_URL.'/orderform.php';
...
if($enc_data != '' && $enc_info != '' && $tran_cd != '') {
    ...
    $sql = " select * from {$g5['g5_contents_order_data_table']} where od_id = '$ordr_idxx' ";
    $row = sql_fetch($sql);
...
<input type="hidden" name="Ret_URL"      value="<?php echo G5_MCONTENTS_URL; ?>/kcp/order_approval_form.php">
<!-- 인증시 필요한 파라미터(변경불가)-->
