...
// 신규회원 쿠폰발생
if($w == '' && $setting['de_member_reg_coupon_use'] && $setting['de_member_reg_coupon_term'] > 0 && $setting['de_member_reg_coupon_price'] > 0) {
    $j = 0;
    $create_coupon = false;
    ...
    if($create_coupon) {
        ...
        $cp_end = date("Y-m-d", (G5_SERVER_TIME + (86400 * ((int)$setting['de_member_reg_coupon_term'] - 1))));
        $cp_type = 0;
        $cp_price = $setting['de_member_reg_coupon_price'];
        $cp_trunc = 1;
        $cp_minimum = $setting['de_member_reg_coupon_minimum'];
        $cp_maximum = 0;
