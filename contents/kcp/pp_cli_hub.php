...
if($tx == 'cash')
    $sql = " select count(*) as cnt from {$g5['g5_contents_cash_table']} where cs_id = '{$_POST['ordr_idxx']}' and cs_taxsave = 1 ";
else
    $sql = " select count(*) as cnt from {$g5['g5_contents_order_table']} where od_id = '{$_POST['ordr_idxx']}' and od_cash = 1 ";
...
<?php
    ...
    /* = -------------------------------------------------------------------------- = */
    include G5_CONTENTS_PATH.'/settle_kcp.inc.php';
    require G5_CONTENTS_PATH.'/kcp/pp_cli_hub_lib.php';
    ...
    $g_conf_site_id   = strlen($setting['de_kcp_mid']) == 3 ? "SR".$setting['de_kcp_mid'] : $setting['de_kcp_mid']; // 리얼 반영시 KCP에 발급된 site_cd 사용 ex) T0000
