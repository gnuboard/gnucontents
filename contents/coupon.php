...
if (G5_IS_MOBILE) {
    include_once(G5_MCONTENTS_PATH.'/coupon.php');
    return;
}
...
<div id="coupon" class="new_win">
    ...
    <div class="tbl_wrap tbl_head01">
        ...
        for($i=0; $row=sql_fetch_array($result); $i++) {
            ...
            if($row['cp_method'] == 1) {
                $sql = " select ca_name from {$g5['g5_contents_category_table']} where ca_id = '{$row['cp_target']}' ";
                $ca = sql_fetch($sql);
                $cp_target = $ca['ca_name'].'의 상품할인';
            } else if($row['cp_method'] == 2) {
                $cp_target = '결제금액 할인';
            } else {
                $sql = " select it_name from {$g5['g5_contents_item_table']} where it_id = '{$row['cp_target']}' ";
                $it = sql_fetch($sql);
