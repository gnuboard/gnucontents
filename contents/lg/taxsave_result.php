...
if($tx == 'cash') {
    ...
    $od_id = $row['cs_id'];
    $od_tno = $row['cs_tno'];
    $goods_name = $row['cs_cash_price'].'원 캐시충전';
    $order_price = (int)$row['cs_receipt_price'];
    $settle_case = $row['cs_settle_case'];
    $od_casseqno = $row['cs_casseqno'];
} else {
    ...
    $order_price = (int)($row['od_receipt_price'] - $row['od_refund_price']);
    $settle_case = $row['od_settle_case'];
    $od_casseqno = $row['od_casseqno'];
}
...
$LGD_CUSTOM_MERTNAME        = $setting['de_admin_company_name'];       //상점명
$LGD_CUSTOM_CEONAME         = $setting['de_admin_company_owner'];      //대표자명
$LGD_CUSTOM_BUSINESSNUM     = $setting['de_admin_company_saupja_no'];  //사업자등록번호
$LGD_CUSTOM_MERTPHONE       = $setting['de_admin_company_tel'];        //상점 전화번호
