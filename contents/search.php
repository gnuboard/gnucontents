...
if (G5_IS_MOBILE) {
    include_once(G5_MCONTENTS_PATH.'/search.php');
    return;
}
...
$sql_common = " from {$g5['g5_contents_item_table']} a, {$g5['g5_contents_category_table']} b ";
...
// 총몇개 = 한줄에 몇개 * 몇줄
$items = $setting['de_search_list_mod'] * $setting['de_search_list_row'];
...
if ($is_admin) {
    echo '<div class="sit_admin"><a href="'.G5_ADMIN_URL.'/contents_admin/configform.php#anc_scf_etc'.'" class="btn_admin">검색 설정</a></div>';
}
?>
...
<div id="ssch">
    ...
    <div id="ssch_frm">
        ...
        <ul id="ssch_sort">
            <li class="sort_first"><a href="#" class="btn01" onclick="set_sort('it_sum_qty', 'desc'); return false;">판매많은순</a></li>
    ...
    <div>
        <?php
        // 리스트 유형별로 출력
        $list_file = G5_CONTENTS_SKIN_PATH.'/'.$setting['de_search_list_skin'];
        if (file_exists($list_file)) {
            define('G5_CONTENTS_CSS_URL', G5_CONTENTS_SKIN_URL);
            $list = new cm_item_list($list_file, $setting['de_search_list_mod'], $setting['de_search_list_row'], $setting['de_search_img_width'], $setting['de_search_img_height']);
