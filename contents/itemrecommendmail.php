...
$recommendmail_count = (int)get_session('ss_cm_recommendmail_count') + 1;
if ($recommendmail_count > 3)
    alert_close('한번 접속후 일정수의 메일만 발송할 수 있습니다.\\n\\n계속해서 메일을 보내시려면 다시 로그인 또는 접속하여 주십시오.');
set_session('ss_cm_recommendmail_count', $recommendmail_count);
...
$it_id = $it['it_id'];
$it_name = $it['it_name'];
$it_mimg = cm_get_it_image($it_id, $setting['de_mimg_width'], $setting['de_mimg_height']);

ob_start();
include G5_CONTENTS_PATH.'/mail/itemrecommend.mail.php';
