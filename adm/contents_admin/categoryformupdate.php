...
if ($w == "")
{
    ...
    $sql = " insert {$g5['g5_contents_category_table']}
                set ca_id   = '$ca_id',
                    ca_name = '$ca_name',
                    $sql_common ";
    sql_query($sql);
...
else if ($w == "d")
{
    ...
    $sql = " select COUNT(*) as cnt from {$g5['g5_contents_category_table']}
              where SUBSTRING(ca_id,1,$len) = '$ca_id'
                and ca_id <> '$ca_id' ";
    $row = sql_fetch($sql);
    if ($row['cnt'] > 0)
    ...
    $str = $comma = "";
    $sql = " select it_id from {$g5['g5_contents_item_table']} where ca_id = '$ca_id' ";
    ...
    // 분류 삭제
    $sql = " delete from {$g5['g5_contents_category_table']} where ca_id = '$ca_id' ";
    sql_query($sql);
}
