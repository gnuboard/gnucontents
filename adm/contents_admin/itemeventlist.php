...
$sql_common = " from {$g5['g5_contents_item_table']} a
                left join {$g5['g5_contents_event_item_table']} b on (a.it_id=b.it_id and b.ev_id='$ev_id') ";
...
// 이벤트제목
if($ev_id) {
    $tmp = sql_fetch(" select ev_subject from {$g5['g5_contents_event_table']} where ev_id = '$ev_id' ");
...
<select name="ev_id" id="ev_id" action="<?php echo $_SERVER['SCRIPT_NAME']; ?>">
    ...
    $sql1 = " select ev_id, ev_subject from {$g5['g5_contents_event_table']} order by ev_id desc ";
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        <th scope="col">이벤트</th>
        <th scope="col"><a href="<?php echo cm_title_sort("a.it_id") . '&amp;'.$qstr1.'&amp;ev_id='.$ev_id; ?>">상품코드</a></th>
        <th scope="col"><a href="<?php echo cm_title_sort("it_name") . '&&amp;'.$qstr1.'&amp;ev_id='.$ev_id; ?>">상품명</a></th>
    </tr>
    </thead>
    <tbody>
    <?php for ($i=0; $row=mysql_fetch_array($result); $i++) {
        ...
        $sql = " select ev_id from {$g5['g5_contents_event_item_table']}
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td><a href="<?php echo $href; ?>"><?php echo cm_get_it_image($row['it_id'], 50, 50); ?> <?php echo cut_str(stripslashes($row['it_name']), 60, "&#133"); ?></a></td>
    </tr>
