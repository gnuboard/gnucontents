...
if ($w == "u")
{
    ...
    $sql = " select * from {$g5['g5_contents_event_table']} where ev_id = '$ev_id' ";
    $ev = sql_fetch($sql);
    if (!$ev['ev_id'])
        alert("등록된 자료가 없습니다.");
    ...
    $sql = " select b.it_id, b.it_name
                from {$g5['g5_contents_event_item_table']} a left join {$g5['g5_contents_item_table']} b on ( a.it_id = b.it_id )
                where a.ev_id = '$ev_id' ";
...
else
{
    ...
    // 1.03.00
    // 입력일 경우 기본값으로 대체
    //$ev['ev_img_width']  = $setting['de_simg_width'];
    //$ev['ev_img_height'] = $setting['de_simg_height'];
    $ev['ev_img_width']  = 230;
...
// 모바일 1줄당 이미지수 필드 추가
if(!sql_query(" select ev_mobile_list_row from {$g5['g5_contents_event_table']} limit 1 ", false)) {
    sql_query(" ALTER TABLE `{$g5['g5_contents_event_table']}`
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
            ...
            <a href="<?php echo G5_CONTENTS_URL; ?>/event.php?ev_id=<?php echo $ev['ev_id']; ?>" class="btn_frmline">이벤트바로가기</a>
            <button type="button" class="btn_frmline contents_event">테마설정 가져오기</button>
        </td>
    </tr>
    <?php } ?>
    <tr>
        ...
        <td>
            <?php echo help('기본으로 제공하는 스킨은 '.str_replace(G5_PATH.'/', '', G5_CONTENTS_SKIN_PATH).'/list.*.skin.php 입니다.'.PHP_EOL.G5_CONTENTS_DIR.'/event.php?ev_id=1234567890&amp;skin=userskin.php 처럼 직접 만든 스킨을 사용할 수도 있습니다.'); ?>
            <select name="ev_skin" id="ev_skin">
                <?php echo cm_get_list_skin_options("^list.[0-9]+\.skin\.php", G5_CONTENTS_SKIN_PATH, $ev['ev_skin']); ?>
            </select>
        </td>
    </tr>
    <tr>
        ...
        <td>
            <?php echo help('기본으로 제공하는 스킨은 '.str_replace(G5_PATH.'/', '', G5_MCONTENTS_SKIN_PATH).'/list.*.skin.php 입니다.'.PHP_EOL.G5_CONTENTS_DIR.'/event.php?ev_id=1234567890&amp;skin=userskin.php 처럼 직접 만든 스킨을 사용할 수도 있습니다.'); ?>
            <select name="ev_mobile_skin" id="ev_mobile_skin">
                <?php echo cm_get_list_skin_options("^list.[0-9]+\.skin\.php", G5_MCONTENTS_SKIN_PATH, $ev['ev_mobile_skin']); ?>
            </select>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
            ...
            <?php
            $mimg_str = "";
            $mimg = G5_DATA_PATH.'/cmevent/'.$ev['ev_id'].'_m';
            if (file_exists($mimg)) {
                ...
                $mimg_str = '<img src="'.G5_DATA_URL.'/cmevent/'.$ev['ev_id'].'_m" width="'.$width.'" alt="">';
            }
            if ($mimg_str) {
    ...
    <tr>
        ...
        <td>
            ...
            <?php
            $himg_str = "";
            $himg = G5_DATA_PATH.'/cmevent/'.$ev['ev_id'].'_h';
            if (file_exists($himg)) {
                ...
                $himg_str = '<img src="'.G5_DATA_URL.'/cmevent/'.$ev['ev_id'].'_h" width="'.$width.'" alt="">';
            }
            if ($himg_str) {
                echo '<div class="banner_or_img">';
                echo $himg_str;
    ...
    <tr>
        ...
        <td>
            ...
            <?php
            $timg_str = "";
            $timg = G5_DATA_PATH.'/cmevent/'.$ev['ev_id'].'_t';
            if (file_exists($timg)) {
                ...
                $timg_str = '<img src="'.G5_DATA_URL.'/cmevent/'.$ev['ev_id'].'_t" width="'.$width.'" alt="">';
            }
            if ($timg_str) {
                echo '<div class="banner_or_img">';
                echo $timg_str;
...
<script>
$(function() {
    $(".contents_event").on("click", function() {
