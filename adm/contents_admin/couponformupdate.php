...
if($_POST['cp_method'] == 0) {
    $sql = " select count(*) as cnt from {$g5['g5_contents_item_table']} where it_id = '$cp_target' and it_nocoupon = '0' ";
    $row = sql_fetch($sql);
    if(!$row['cnt'])
        alert('입력하신 상품코드는 존재하지 않는 코드이거나 쿠폰적용안함으로 설정된 상품입니다.');
} else if($_POST['cp_method'] == 1) {
    $sql = " select count(*) as cnt from {$g5['g5_contents_category_table']} where ca_id = '$cp_target' and ca_nocoupon = '0' ";
    $row = sql_fetch($sql);
    if(!$row['cnt'])
...
if($w == '') {
    ...
    $j = 0;
    do {
        $cp_id = cm_get_coupon_id();

        $sql3 = " select count(*) as cnt from {$g5['g5_contents_coupon_table']} where cp_id = '$cp_id' ";
        $row3 = sql_fetch($sql3);
    ...
    $sql = " INSERT INTO {$g5['g5_contents_coupon_table']}
    ...
    $sql = " select * from {$g5['g5_contents_coupon_table']} where cp_id = '$cp_id' ";
    $cp = sql_fetch($sql);
    ...
    $sql = " update {$g5['g5_contents_coupon_table']}
...
if($w == '' && ($_POST['cp_sms_send'] || $_POST['cp_email_send'])) {
    ...
    for($i=0; $i<$count; $i++) {
        ...
        if($config['cf_email_use'] && $_POST['cp_email_send'] && $arr_send_list[$i]['mb_email'] && $arr_send_list[$i]['mb_mailling']) {
            ...
            ob_start();
            include G5_CONTENTS_PATH.'/mail/couponmail.mail.php';
