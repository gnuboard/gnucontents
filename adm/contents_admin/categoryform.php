...
$sql_common = " from {$g5['g5_contents_category_table']} ";
if ($is_admin != 'super')
...
if ($w == "")
{
    ...
    $sql = " select MAX(SUBSTRING(ca_id,$len2,2)) as max_subid from {$g5['g5_contents_category_table']}
    ...
    else // 1단계 분류
    {
        ...
        $ca['ca_use'] = 1;
        $ca['ca_explan_html'] = 1;
        $ca['ca_img_width']  = $setting['de_simg_width'];
        $ca['ca_img_height'] = $setting['de_simg_height'];
        $ca['ca_mobile_img_width']  = $setting['de_simg_width'];
...
if(!$ca['ca_skin_dir'])
    ...
    if(preg_match('#^theme/(.+)$#', $ca['ca_skin_dir'], $match))
        ...
        $g5_contents_skin_path  = G5_PATH.'/'.G5_SKIN_DIR.'/contents/'.$ca['ca_skin_dir'];
}
...
if(!$ca['ca_mobile_skin_dir'])
    ...
    if(preg_match('#^theme/(.+)$#', $ca['ca_mobile_skin_dir'], $match))
        ...
        $g5_mcontents_skin_path = G5_MOBILE_PATH.'/'.G5_SKIN_DIR.'/contents/'.$ca['ca_mobile_skin_dir'];
}
?>
    ...
    <div class="tbl_frm01 tbl_wrap">
        ...
        <tr>
            ...
            <?php } else { ?>
                ...
                <a href="<?php echo G5_CONTENTS_URL; ?>/list.php?ca_id=<?php echo $ca_id; ?>" class="btn_frmline">미리보기</a>
        ...
        <tr>
            ...
            <td colspan="3">
                <?php echo get_skin_select('contents', 'ca_skin_dir', 'ca_skin_dir', $ca['ca_skin_dir']); ?>
            </td>
        </tr>
        <tr>
            <th scope="row"><label for="ca_mobile_skin_dir">모바일용 스킨명</label></th>
            <td colspan="3">
                <?php echo get_mobile_skin_select('contents', 'ca_mobile_skin_dir', 'ca_mobile_skin_dir', $ca['ca_mobile_skin_dir']); ?>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <?php echo help('기본으로 제공하는 스킨은 '.str_replace(G5_PATH.'/', '', $g5_contents_skin_path).'/list.*.skin.php 입니다.'); ?>
                <select id="ca_skin" name="ca_skin">
                    <?php echo cm_get_list_skin_options("^list.[0-9]+\.skin\.php", $g5_contents_skin_path, $ca['ca_skin']); ?>
                </select>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <?php echo help("컨텐츠몰환경설정 &gt; 이미지(소) 넓이가 기본값으로 설정됩니다.\n".G5_CONTENTS_URL."/list.php에서 출력되는 이미지의 폭입니다."); ?>
        ...
        <tr>
            ...
            <td>
                <?php echo help("컨텐츠몰환경설정 &gt; 이미지(소) 높이가 기본값으로 설정됩니다.\n".G5_CONTENTS_URL."/list.php에서 출력되는 이미지의 높이입니다."); ?>
        ...
        <tr>
            ...
            <td>
                <?php echo help('기본으로 제공하는 스킨은 '.str_replace(G5_PATH.'/', '', $g5_mcontents_skin_path).'/list.*.skin.php 입니다.'); ?>
                <select id="ca_mobile_skin" name="ca_mobile_skin">
                    <?php echo cm_get_list_skin_options("^list.[0-9]+\.skin\.php", $g5_mcontents_skin_path, $ca['ca_mobile_skin']); ?>
                </select>
            </td>
        </tr>
        <tr>
...
<?php echo preg_replace('#</div>$#i', '<button type="button" class="contents_category">테마설정 가져오기</button></div>', $frm_submit); ?>
...
$(function() {
    $(".contents_category").on("click", function() {
