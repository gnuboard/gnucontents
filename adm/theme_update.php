...
if($_POST['set_default_skin'] == 1) {
    $keys = 'set_default_skin, cf_member_skin, cf_mobile_member_skin, cf_new_skin, cf_mobile_new_skin, cf_search_skin, cf_mobile_search_skin, cf_connect_skin, cf_mobile_connect_skin, cf_faq_skin, cf_mobile_faq_skin, qa_skin, qa_mobile_skin, de_contents_skin, de_contents_mobile_skin';
    ...
    if($tconfig['set_default_skin']) {
        ...
        if(!empty($de_sql_common)) {
            $sql = " update {$g5['g5_contents_default_table']} set " . implode(', ', $de_sql_common);
            sql_query($sql);
        }
    }
}
